...
cdb.ui.common.SlideshowModel = Backbone.Model.extend({
  ...
  prev: function() {
    ...
    if (i < 0) i = this.parent.collection.length - 1;
    this.set("currentIndex", i);
  },

  next: function() {
    var i = this.get("currentIndex") + 1;
    if (i >= this.parent.collection.length) i = 0;
    this.set("currentIndex", i);
  }
...
cdb.ui.common.Slideshow = cdb.core.View.extend({
  ...
  initialize: function() {
    _.bindAll(this, 'render', 'prev', 'next', 'gotoSlide');
  ...
  gotoSlide: function() {
    var previous = this.model.previous("currentIndex");
